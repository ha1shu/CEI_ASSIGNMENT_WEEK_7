...
|--------------------|--------|---------------------------------------------------------------------------|
| `superstore_bronze` | Bronze | Raw, unclean data loaded directly from `Superstore.csv`. No transformations applied — kept as-is for audit/reprocessing. |
...
| `Incremental_load.csv`  | 2.19 MB | Simulated new/incremental data — contains a mix of records that update existing rows and records that insert new rows into `superstore_silver`. |
